...
export default {
    ...
    methods: {
         ...
         */
        onSubmit() {
            console.log("ON SUBMIT FILES1 =", this.$refs.fileInput.files);
            if (this.$refs.fileInput.files) {
                this.files = this.$refs.fileInput.files;
                console.log("ON SUBMIT FILES2 =", this.files);
                this.submit();
            }
        },
        /**
        ...
        buildRequest(file) {
            ...
            console.log("BUILDREQ PATH =", this.path + '/' + file.name);
            console.log("BUILDREQ FILE =", file);

            formData.append('path', this.path + '/' + file.name);
            formData.append('file', file);

            return this.$api.upload(this.path + '/' + file.name, file).then(response => {
