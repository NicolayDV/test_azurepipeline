...
export class MM {
    ...
    init() {
        ...
        let mm = this;
        Vue.use(Vuex);
        // console.log("PATH2 =", this.options.basePath, this.options.path);
                ...
                addSelected(state, file) {
                    if (state.options.input.multiple) {
                        if (!Array.isArray(state.selected)) {
                            ...
                            let index = state.selected.findIndex(element => { return element.path_lower === file.path_lower; });
                            if (index>-1) return;
                        }
            ...
            getters: {
                isSelected: (state, getters) => (file) => {
                    ...
                    if (state.options.input.multiple) {
                        ...
                        return (state.selected && state.selected.path_lower === file.path_lower);
                    }
                },
        ...
        if (this.input) {
            if (e.selected) {
                if (this.input.multiple) {
                    let selected = e.selected.map(element => { return element.path_lower; });
                    this.input.el.value = selected.join("\n");
                } else {
                    this.input.el.value = e.selected.path_lower;
                }
            } else {
                this.input.el.value = '';
            }
